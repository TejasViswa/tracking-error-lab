...
This shows exponential decay in magnitude with lag. When $\phi < 0$, correlations decay with alternating signs.
...
::: {.callout-note}
**Note on conventions:** Some texts use $1/(T-h)$ instead of $1/T$ in the denominator. Both conventions are asymptotically equivalent, and $1/T$ is commonly used in long-run variance estimation.
:::

### 5.4 Bandwidth selection

Common rules of thumb for choosing the lag truncation parameter $L$:

- **Simple power rules:** $L \approx T^{1/4}$ or $L \approx 0.75 T^{1/3}$
- **Data-dependent (Andrews 1991):** Plug-in automatic bandwidth selection based on estimated autocorrelation structure
- **Conservative choice:** $L \approx \lfloor 4(T/100)^{2/9} \rfloor$ often recommended for AR-type processes

For daily financial data with $T=252$, typical choices: $L \in [6, 10]$
...
1. **[Overview](index.html)** — Start here for intuition and practical implications
2. **[Intuitive Math](intuitive_math.html)** — Complete derivation of variance-of-a-sum formula
3. **[Technical Math](technical_math.html)** — Advanced tools: AR(1) models, Newey-West estimation, closed-form solutions
4. **[Interactive App](https://tracking-error-lab-kgkju98o4pqxdjoevqtsay.streamlit.app/)** — Experiment with your own data
